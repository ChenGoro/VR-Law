...
public class HorizontalObjectLayoutGroup : MonoBehaviour
{
    ...
    private void CircularLayout(HorizontalObjectLayoutGroupMember[] activeMembers)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            if (rotateToFaceCenter)
            {
                ...
                Log($"Set rotation of {activeMembers[i].name} to face center: {rotation}");
            }
            Log($"Set the position of {activeMembers[i].name} to {position}");
        }
        Log($"Circular layout applied with radius {radius} and arc length {arcLength}");
    }
    ...
    private void LinearLayout(HorizontalObjectLayoutGroupMember[] activeMembers)
    {
        ...
        Log($"Linear layout applied with alignment {alignment} and element width {elementWidth} and spacing {elementSpacing}");
    }
    ...
    public void SetUseGroupLayout(bool value)
    {
        ...
        Log($"Set useGroupLayout to {value}");
    }
    ...
    {
        Log($"Resetting layout to original positions");
        if (originalPositions != null && originalPositions.Length > 0)
        {
            for (int i = 0; i < groupMembers.Length; i++)
            {
                ...
                Log($"Reset position of {groupMembers[i].name} to original position: {originalPositions[i]}");
            }
        }
    }
    ...
    public void StoreOriginalPositions()
    {
        ...
        for (int i = 0; i < groupMembers.Length; i++)
        {
            ...
            Log($"Stored original position of {groupMembers[i].name}: {originalPositions[i]}");
        }
    ...
    {
        if (showDebugLogs) Debug.Log("[HorizontalObjectLayoutGroup] " + msg);
    }
